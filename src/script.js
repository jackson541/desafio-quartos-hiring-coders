...
divQuartos = document.getElementById('quartos')


function adicionarQuartos (inicio=0, fim=24) {
  ...
  for (i=inicio; i<fim; i++){
    ...
    quarto.classList.add("card")
    div.classList.add("card-body")

    foto.setAttribute('src', quartosCadastrados[i].photo)
    foto.setAttribute('height', '200px')
    foto.classList.add("card-img-top")
    quarto.appendChild(foto)

    nome.innerHTML = quartosCadastrados[i].name
    nome.classList.add("card-title")
    div.appendChild(nome)

    tipo.innerHTML = "Tipo: " + quartosCadastrados[i].property_type
    tipo.classList.add("card-text", quartosCadastrados[i].property_type)
    div.appendChild(tipo)

    preco.innerHTML = "R$ " + quartosCadastrados[i].price + ",00 por noite"
    preco.classList.add("card-text")
    div.appendChild(preco)

    quarto.appendChild(div)
    
    coluna.classList.add('col-3', 'mb-5')
    coluna.appendChild(quarto)

    divQuartos.appendChild(coluna);
  }
}
...
function listarApartamentos() {
  adicionarQuartos(0, 2)
}

function listarCasas() {
  adicionarQuartos(2, 4)
}

function listarChacaras() {
  adicionarQuartos(4, 7)
}

function listarEstudios() {
  adicionarQuartos(7, 11)
}

function listarLofts() {
  adicionarQuartos(11, 13)
}

function listarQuartos() {
  adicionarQuartos(13, 21)
}

function listarSitios() {
  adicionarQuartos(21, 23)
}
